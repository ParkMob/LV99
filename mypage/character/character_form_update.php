...
if ($ad['ad_use_body']) {
	...
	if($_FILES['ch_music_file']['name']) {
		// 확장자 따기
		$exp = explode(".", $_FILES['ch_music_file']['name']);
		$exp = $exp[count($exp)-1];

		$image_name = "music_".time().".".$exp;
		upload_file($_FILES['ch_music_file']['tmp_name'], $image_name, $character_image_path);
		$ch_music= $character_image_url."/".$image_name;
	}
	// 이미지 무차별 삭제 방어
	if(strstr($ch_music, G5_URL)) {
		// 본 사이트의 경로를 가지고 있을 경우
		if(!strstr($ch_music, $character_image_url)) {
			// 본인의 캐릭터 경로가 아닐 경우
			alert("이미지 경로가 올바르지 않습니다.");
		}
	}
	$sql_article .= " , ch_music		= '{$ch_music}'";
...
goto_url(G5_URL.'/member/viewer.php?ch_id='.$ch_id, false);
?>
