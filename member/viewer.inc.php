...
<div class="cmt_list">
    <div class="character_cmt quoto">“</i></div>
...
<div class="visual-area">
	<div id="character_body">
		...
		<div class="char_profile">
			<div class="char_info_list">
				...
				<div class="char_info_detail">
					...
					<div class="char_info age"><?=$ch['age']?>세</div>
				</div>
				<div class="char_info_detail">
					<div class="char_info cate highlight">신장/체중</div>
					<div class="char_info height"><?=$ch['height']?>cm / </div>
					<div class="char_info weight"><?=$ch['weight']?>kg</div>
            	</div>
				<div class="char_info_detail">
					<div class="char_info cate highlight">생일</div>
					<div class="char_info birth"><?=$ch['birth']?></div>
				</div>
			</div>
			
			<div class="char_info_list">
				<a href="<?=G5_URL?>/member/index.php?side=<?=$ch['ch_side']?>"> <div class="char_name side highlight"><?=get_side_name($ch['ch_side'])?></div> </a>
        ...
        <? if($ch['ch_music']) { ?>
                <div class="closet_profile" style="margin-right:50px;">
                <audio src=" <?=$ch['ch_music']?>" controls  loop id="theme-music" style="display:none"><?php echo $view['wr_subject'] ?></audio>
                <a href="#" id="btnPlay" ><i class="fas fa-play"></i></a>
                <a href="#" id="btnPause"><i class="fas fa-stop" ></i></a>
                
<!--                 
                <a href="#"><i class="fas fa-pause" id="btnPause"></i></a> -->

                </div>
			<? } ?>

		<div class="body_img">
			<img src="<?=$ch['ch_thumb']?>" id="body_img_raw"alt="캐릭터 전신"  onclick="wrapWindowByMask(this.src)" />
		</div>
...
<? } ?>
	...
	<div class="body_img" style="margin:auto;">
			<img src="<?=$ch['ch_body']?>" id="body_img_raw"alt="캐릭터 전신"  onclick="wrapWindowByMask(this.src)" />
		</div>
	<?}?>
<!-- //캐릭터 비쥬얼 (이미지) 출력 영역 -->
    <hr class="padding" />
...
<section class="section section--menu" style="display:flex;">
				<nav class="menu menu--viola">
					<ul class="menu__list">
						<li class="menu__item menu__item--current"><a href="#" class="menu__link" id="appr" >외관</a></li>
						<li class="menu__item" id="per" ><a href="#" class="menu__link">성격</a></li>
						<li class="menu__item" id="etc"><a href="#" class="menu__link">기타</a></li>
					</ul>
				</nav>
</section>

<div class="profile-box appr default">
	<div class="profile-title">APPEARANCE</div>
...
<div class="profile-title">PERSONALITY</div>
<div class="theme-box profile">
<?=nl2br($ch['per'])?>
</div>
</div>
...
<div class="profile-title">ETC</div>
<div class="theme-box profile">
<?=nl2br($ch['etc'])?>
</div>
</div>
...
<hr class="padding" />
	...
	<? if($is_admin){?>
    <a href='<?=G5_URL?>/mypage/character/character_form.php?w=u&ch_id=<?=$ch['ch_id']?>' class="ui-btn point">수정</a>
		<? }?>
    <a href='<?=G5_URL?>/member' class="ui-btn">목록</a>
...
btnPlay.onclick = function (){ myAudio.play(); console.log("play");}
btnPause.onclick = function () { myAudio.pause(); myAudio.currentTime = 0; }
btnStop.onclick = function () {  console.log("stop"); myAudio.pause(); console.log("stop");}
