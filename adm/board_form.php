...
<section id="anc_001">
	...
	<div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
			<th scope="row"><label for="bo_order">게시판 순서<strong class="sound_only">필수</strong></label></th>
			<td colspan="2">
            <?php echo help('숫자가 작을수록 메뉴의 앞으로 옵니다.') ?>
				<input type="text" name="bo_order" value="<?php echo get_text($board['bo_order']) ?>" id="bo_order" required class="required frm_input" size="3" maxlength="12">
			</td>
		</tr>
		<?php if ($w == 'u') { ?>
		<tr>
		...
		<?php } ?>
		<tr>
			<th scope="row"><label for="bo_order_whippy">메뉴 표시 </label></th>
			<td colspan="2">
				<?php echo help('게시판 목록을 메뉴에 보이고 싶을 경우 체크하세요') ?>
                <input type="checkbox" name="bo_order_whippy" value="1" <?php echo $board['bo_order_whippy']?'checked':''; ?> id="bo_order_whippy">
			</td>
		</tr>
        <tr>
			<th scope="row"><label for="bo_1">멤버란</label></th>
			<td colspan="2">
				<?php echo help('멤버란으로 사용시 체크하세요.') ?>
                <input type="checkbox" name="bo_1" value="1" <?php echo $board['bo_1']?'checked':''; ?> id="bo_1">
			</td>
		</tr>
        <tr>
			<th scope="row"><label for="bo_2">페이지 표시</label></th>
			<td colspan="2">
				<?php echo help('페이지 연결을 원할 경우 페이지 아이디를 적어주세요') ?>
                <input type="text" name="bo_2" value="<?php echo get_text($board['bo_2']) ?>" id="bo_2" class="frm_input" size="10" maxlength="20">
			</td>
		</tr>
		</tbody>
		</table>
