...
function get_couple_select($selected="")
{
	global $g5;

	$sql = " select * from {$g5['couple_table']}";
	$result = sql_query($sql);
	$str = "<select id= 'cf_1' name='cf_1'>\n";
	for ($i=0; $row=sql_fetch_array($result); $i++) {

        $str .= '<option value="'.$row['co_id'].'"';
        if ($row['co_id'] == $selected) $str .= ' selected';
        $str .= '>'.$row['co_title'].'</option>';
