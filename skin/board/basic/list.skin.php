...
<div class = "board_category">
 <?php if ($is_category) { 
	...
	<?php } 
        ...
        <a href="<?=G5_BBS_URL?>/board.php?bo_table=<?=$bo_table?>">전체</a>
    <?}
}?> 
</div>
		...
		<li class="theme-box <? if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
		  <!-- <?php if ($is_checkbox) { ?>
			...
			<a href="<? echo $list[$i]['href'] ?>">
				...
				<strong><?if(strstr($list[$i]['wr_option'], 'secret')){?><span ><i class="fas fa-lock"></i></span><?}
                else if($list[$i]['wr_secret']){?><span ><i class="fas fa-user-tag"></i></span><?}else if($list[$i]['wr_protect']!=''){?><span><i class="fas fa-key"></i></span><?}else {}?>
				...
				<div class="info">
					...
					<span class="date">
					<? echo date('Y-m-d H:i ', strtotime($list[$i]['wr_datetime'])) ?>
					</span>
				</div>
			</a> 
		</li>
