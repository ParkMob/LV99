...
$c_option = '';
	...
	if ($c_is_html || $c_is_secret ) {
		$c_option = '';
		if ($c_is_html) {
				$c_option .= "\n".'<span><input type="checkbox" class="html" id="wr_html_'.$list_item['wr_id'].'" name="html" value="html2">'."\n".'<label for="wr_html_'.$list_item['wr_id'].'">html</label></span>';

         
		}else {
            $c_option_hidden .= '<input type="hidden" name="wr_option" value="html2">';
        }

		if ($c_is_secret) {
			if ($is_admin || $c_is_secret==1) {

				$c_option .= "\n".'<span><input type="checkbox" class="secret" id="wr_secret_'.$list_item['wr_id'].'" name="wr_secret" value="secret">'."\n".'<label for="wr_secret_'.$list_item['wr_id'].'">비밀글</label></span>';
			} else {
...
<aside class="bo_vc_w" id="bo_vc_w_<?=$list_item['wr_id']?>">
	...
	<div class="input-comment"> 
		...
		**********************************************/?>
		...
		<div class="action-check form-input" style="margin-top:10px;"> 
		...
		<input type="password" name="wr_password" value="" placeholder="비밀번호" style="max-width:40%" />
        </div>
	<?}?>
		...
		<? } ?> 
			...
			<span>
			<a href="<?php echo $board_skin_url ?>/emoticon_list.php" class="new_win" onclick="window.open('<?php echo $board_skin_url ?>/emoticon_list.php', 'emoticon', 'width=400, height=600');return false;">[이모티콘]</a>
			</span>
