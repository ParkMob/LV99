			...
			<dl>
				<dt>
					<select name="wr_type" onchange="fn_log_type(this.value);">
						...
						<option value="VIDEO" <?=$write['wr_type'] == "TWITTER" ? "selected" : ""?>>TWEET</option>
				...
				<dd>
                    ...
                    <div id="add_IMGUR" <?=($write['wr_type'] == "IMGUR") ? "":"style='display: none;'"?>>
                        <input type="file" id="wr_imgur_raw" style='width: 80%;' class="frm_file frm_input view_image_area"/>
                      
                        <button type="button" class="ui-btn point" id="imgur-upload" style="width:19%;">이미지 등록</button>
					</div>
                    
                    <input type="text" name="wr_10" id="wr_delete" value="<?=$write['wr_10']?>" style="display:none">

					<div id="add_VIDEO" <?=($write['wr_type'] == "VIDEO"||$write['wr_type'] == "TWITTER") ? "" : "style='display: none;'"?>>
	...
	if ($is_notice || $is_html || $is_secret || $is_mail) {
		...
		$sec .='<option value="member" '.$mem_select.'>멤버공개</option>';}
					...
					<? if(!$write['wr_dice1']) { if(!$board['bo_use_chick']){ ?>
							<input type="checkbox" id="game" name="game" value="dice" /> <label for="game">주사위</label>
					<? }} else { 
					?>
