...
<!-- 게시물 읽기 시작 { -->
	...
	<div class="bo_v_nb">
		<? if ($prev['wr_id']) { ?><a href="javascript:open_view('<?=$bo_table?>','<?=$prev['wr_id']?>');" class="prev" id="prev" name=<?=$prev['wr_id']?> ><span>이전글</span></a><? } ?>
		<? if ($next['wr_id']) { ?><a href="javascript:open_view('<?=$bo_table?>','<?=$next['wr_id']?>');" class="next" id="next" name=<?=$next['wr_id']?>><span>다음글</span></a><? } ?>
	</div>
 ...
 if($is_viewer){?>
    ...
    <section id="bo_v_atc" class="txt-center"> 
		...
		if ($view['wr_file']>0) {  /* view 인 경우 */ 
			...
			?>
				<p><img id="img-view" src="<?=$file_url?>" onclick="wrapWindowByMask('<?=$file_url?>')"></p>				
                
			<?}
		}
        else if($view['wr_url']){
            $file_url=$view['wr_url'];?>
          	<p><img  id="img-view" src="<?=$file_url?>" onclick="wrapWindowByMask('<?=$file_url?>')"></p>	<?	
        }
...
<?if ($view['wr_content']){?>
        <div style="margin-bottom:10px;"> <span class="sound_only">내용</span> <?php echo $view['wr_content'] ?></div>
