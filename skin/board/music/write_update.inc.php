...
if($cookie){
	setcookie('MMB_PW', $_POST['wr_password'], 31); //@200912
	setcookie('MMB_NAME', $_POST['wr_name'], 31); //@200912
}
else {
	setcookie('MMB_PW','');
	setcookie('MMB_NAME','');
	...
	if(!isset($temp['wr_reply_more'])){
		sql_query(" ALTER TABLE `{$write_table}` ADD `wr_reply_more` int(11) NOT NULL DEFAULT '0' AFTER `wr_height` ");
		} 
	...
	unset($temp);
	$customer_sql .= ",wr_reply_more = '{$wr_reply_more}', wr_video = '{$wr_video}', wr_text= '{$wr_text}'";
