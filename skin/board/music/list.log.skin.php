...
if($list_item['wr_type'] == 'UPLOAD') { 
	...
	$image_url = '<img class="log_img" src="'.$thumb['src'].'"/>';
	$image_width = $thumb['width'];
	$image_height = $thumb['height'];
} else if($list_item['wr_type'] == 'URL') {
	// URL 형태로 로그를 등록 하였을 때
	$image_url = '<img class="log_img" src="'.$list_item['wr_url'].'">';
	$image_width = $list_item['wr_width'];
	$image_height = $list_item['wr_height'];
}else if($list_item['wr_type'] == 'IMGUR') {
	// imgur 형태로 로그를 등록 하였을 때
	$image_url = '<img class="log_img" src="'.$list_item['wr_url'].'">';
	$image_width = $list_item['wr_width'];
	$image_height = $list_item['wr_height'];
}
else if($list_item['wr_type'] == 'TWEET') {
	// tweet 형태로 로그를 등록 하였을 때
			...
			<div class="pic-header">
				...
				<? if($is_viewer) { 
					...
					if ($delete_href)		{ ?><a href="<?php echo $delete_href ?>" class="del" onclick="del(this.href); return false;">삭제</a><?	} ?>
												<a href="?bo_table=<?=$bo_table?>&log=<?=$list_item['wr_num'] * -1?>&single=Y" target="_blank" class="new"><i class="fas fa-link"></i></a>
					<? if ($update_href)	{ ?><a href="<?php echo $update_href ?>" class="mod"><i class="fas fa-edit"></i></i></a><? } ?>
				<? } ?>

					<div class="co-header">
		<? // 로그 작성자와 코멘트 작성자가 동일할 경우, class='owner' 를 추가한다. ?>
	
		<p class="owner" style="     margin-bottom: 20px;     font-size: 1.5em;">
		
			<strong><?=$list_item['wr_1']?></strong>
		</p>
	</div>
